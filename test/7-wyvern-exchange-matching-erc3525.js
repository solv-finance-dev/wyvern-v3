...
const TestERC3525 = artifacts.require('TestERC3525')
...
contract('WyvernExchange', (accounts) =>
	...
	// 	await Promise.all([erc3525.setApprovalForAll(proxy1,true,{from: account_a}),erc3525.setApprovalForAll(proxy2,true,{from: account_b})])
		
	// 	let vouchers = [{tokenId:1, slot:1111,balance:2e10, sell: 1e10},{tokenId:2, slot:1111,balance:2e10, sell: 1e10}]
	// 	let newVouchers = [{tokenId:100000001, slot:1111,balance:1e10},{tokenId:100000002, slot:1111,balance:1e10}]
	// 	await Promise.all([erc3525.mint(vouchers[0].slot,vouchers[0].tokenId,vouchers[0].balance, {from: account_a}),erc3525.mint(vouchers[1].slot, vouchers[1].tokenId, vouchers[1].balance, {from: account_b})])
		
	// 	const erc3525c = new web3.eth.Contract(erc3525.abi, erc3525.address)
	// 	const selector = web3.eth.abi.encodeFunctionSignature('swapOneForOneERC3525(bytes,address[7],uint8[2],uint256[6],bytes,bytes)')
		
	// 	const paramsOne = web3.eth.abi.encodeParameters(
	// 		['address[2]', 'uint256[2]', 'uint256[2]'],
	// 		[[erc3525.address, erc3525.address], [vouchers[0].tokenId, vouchers[1].tokenId], [vouchers[0].sell, vouchers[1].sell]]
	// 		)

	// 	const paramsTwo = web3.eth.abi.encodeParameters(
	// 		['address[2]', 'uint256[2]', 'uint256[2]'],
	// 		[[erc3525.address, erc3525.address], [vouchers[1].tokenId, vouchers[0].tokenId], [vouchers[1].sell, vouchers[0].sell]]
	// 		)
	...
	// 	const firstData = erc3525c.methods.safeTransferFrom(account_a, account_b, vouchers[0].tokenId, vouchers[0].sell, "0x").encodeABI() + ZERO_BYTES32.substr(2)
	// 	const secondData = erc3525c.methods.safeTransferFrom(account_b, account_a, vouchers[1].tokenId, vouchers[1].sell, "0x").encodeABI() + ZERO_BYTES32.substr(2)
				
	// 	const firstCall = {target: erc3525.address, howToCall: 0, data: firstData}
	// 	const secondCall = {target: erc3525.address, howToCall: 0, data: secondData}
	...
	// 	let [new_balance1,new_balance2] = await Promise.all([erc3525.unitsInToken(vouchers[1].tokenId),erc3525.unitsInToken(vouchers[0].tokenId)])
	...
	// it('matches voucher-voucher swap order, abi-decoding instead',async () =>
	// 	{
	// 	let account_a = accounts[0]
	// 	let account_b = accounts[6]
	...
	// 	const erc3525c = new web3.eth.Contract(erc1155.abi, erc1155.address)
	...
	// 	const firstData = erc3525c.methods.safeTransferFrom(account_a, account_b, nfts[0].tokenId, nfts[0].amount, "0x").encodeABI() + ZERO_BYTES32.substr(2)
	// 	const secondData = erc3525c.methods.safeTransferFrom(account_b, account_a, nfts[1].tokenId, nfts[1].amount, "0x").encodeABI() + ZERO_BYTES32.substr(2)
	...
	// it('matches voucher + erc20 <> voucher orders, matched left, real static call',async () => 
	// 	{
	// 	let account_a = accounts[0]
	// 	let account_b = accounts[6]
	...
	// 	let price = 10000
	// 	let slot = 88888
	// 	let vouchers = [{tokenId:10,value: 1000},{tokenId:11,value:10000}]
		
	// 	let {atomicizer, exchange, registry, statici} = await deploy_core_contracts()
	// 	let [erc20,erc3525] = await deploy([TestERC20,TestERC3525])
	...
	// 	await Promise.all([erc20.approve(proxy1,price,{from: account_a}),erc3525.setApprovalForAll(proxy1,true,{from: account_a}),erc3525.setApprovalForAll(proxy2,true,{from: account_b})])
	// 	await Promise.all([erc20.mint(account_a,price),erc3525.mint(slot, vouchers[0].value),erc3525.mint(slot, vouchers[1].value)])
	...
	// 	const firsterc3525call = erc3525c.methods.safeTransferFrom(account_a, account_b, tokenId, 1, "0x").encodeABI() + ZERO_BYTES32.substr(2)
	// 	const firstData = atomicizerc.methods.atomicize(
	// 	  [erc20.address, erc1155.address],
	// 	  [0, 0],
	// 	  [(firstERC20Call.length - 2) / 2, (firsterc3525call.length - 2) / 2],
	// 	  firstERC20Call + firsterc3525call.slice(2)
	// 	).encodeABI()
		
	// 	const seconderc3525call = erc3525c.methods.safeTransferFrom(account_b, account_a, tokenId, 1, "0x").encodeABI() + ZERO_BYTES32.substr(2)
	// 	const secondData = atomicizerc.methods.atomicize(
	// 	  [erc1155.address],
	// 	  [0],
	// 	  [(seconderc3525call.length - 2) / 2],
	// 	  seconderc3525call
	// 	).encodeABI()
	...
	// 	})
	const erc20_erc3525_match_right_static_call = async (maximumFill,fillCount) => 
	{
	let account_a = accounts[0]
	let account_b = accounts[6]
	...
	await Promise.all([erc20.mint(account_b,price*maximumFill*fillCount),erc3525.mint(slot, tokenId,maximumFill, {from: account_a})])
	await Promise.all([erc20.approve(proxy2,price*maximumFill*fillCount,{from: account_b}),erc3525.setApprovalForAll(proxy1,true,{from: account_a})])
	...
	const aEDParams = web3.eth.abi.encodeParameters(['address', 'uint256', 'uint256'], [erc3525.address, tokenId, sellValue])
	const aEDSelector = web3.eth.abi.encodeFunctionSignature('transferERC3525Exact(bytes,address[7],uint8,uint256[6],bytes)')

	const bEDParams = web3.eth.abi.encodeParameters(['address', 'uint256'], [erc20.address, price*sellValue])
	const bEDSelector = web3.eth.abi.encodeFunctionSignature('transferERC20Exact(bytes,address[7],uint8,uint256[6],bytes)')
	...
	const extradataTwo = '0x'
	const one = {registry: registry.address, maker: account_a, staticTarget: statici.address, staticSelector: selectorOne, staticExtradata: extradataOne, maximumFill: '100', listingTime: '0', expirationTime: '10000000000', salt: '3358'}
	const two = {registry: registry.address, maker: account_b, staticTarget: statici.address, staticSelector: selectorTwo, staticExtradata: extradataTwo, maximumFill: '15', listingTime: '0', expirationTime: '10000000000', salt: '3339'}
	...
	const firstERC3525call = erc3525c.methods.safeTransferFrom(account_a, account_b, tokenId, sellValue, "0x").encodeABI() + ZERO_BYTES32.substr(2)
	const firstData = atomicizerc.methods.atomicize(
		[erc3525.address],
		[0],
	...
	const secondERC20Call = erc20c.methods.transferFrom(account_b, account_a, price*sellValue).encodeABI()
	const secondData = atomicizerc.methods.atomicize(
		[erc20.address],
		[0],
	...
	for (let i = 0 ; i < fillCount ; ++i) {
		console.log('##### ' + await exchange.inst.fills(account_a, await exchange.inst.hashOrder_(one.registry, one.maker, one.staticTarget, one.staticSelector, one.staticExtradata, one.maximumFill, one.listingTime, one.expirationTime, one.salt)))
		await exchange.atomicMatchWith(one, sig, firstCall, two, twoSig, secondCall, ZERO_BYTES32,{from:account_b})
		console.log('##### ' + await exchange.inst.fills(account_a, await exchange.inst.hashOrder_(one.registry, one.maker, one.staticTarget, one.staticSelector, one.staticExtradata, one.maximumFill, one.listingTime, one.expirationTime, one.salt)))
	
		let from_balance = await erc3525.unitsInToken(tokenId)
		let to_balance = await erc3525.unitsInToken(tokenId + 100000000)
		console.log("----" + i + "----")
		console.log("from balance", from_balance.toNumber())
		console.log("to balance", to_balance.toNumber())
	}
	let from_balance = await erc3525.unitsInToken(tokenId)
	let to_balance = await erc3525.unitsInToken(tokenId + 100000000)
	assert.isTrue(to_balance.toNumber() > 0,'Incorrect balance')
	assert.equal(to_balance.toNumber(),  sellValue*fillCount, 'Incorrect balance')
	assert.equal((await erc20.balanceOf(account_b)).toNumber(), price*fillCount*sellValue, 'Incorrect balance')
	}		

		/*
	const erc3525_erc20_match_right_static_call = async (maximumFill,fillCount) => 
		{
		let account_a = accounts[0]
		let account_b = accounts[6]
		...
		await Promise.all([erc20.approve(proxy1,price*maximumFill*fillCount,{from: account_a}),erc3525.setApprovalForAll(proxy2,true,{from: account_b})])
		await Promise.all([erc20.mint(account_a,price*maximumFill*fillCount),erc3525.mint(slot, tokenId,maximumFill, {from: account_b})])
		...
		const selectorOneA = web3.eth.abi.encodeFunctionSignature('sequenceExact(bytes,address[7],uint8,uint256[6],bytes)')
		const selectorOneB = web3.eth.abi.encodeFunctionSignature('sequenceExact(bytes,address[7],uint8,uint256[6],bytes)')
		const aEDParams = web3.eth.abi.encodeParameters(['address', 'uint256'], [erc20.address, price*sellValue])
		...
		const bEDParams = web3.eth.abi.encodeParameters(['address', 'uint256', 'uint256'], [erc3525.address, tokenId, sellValue])
		const bEDSelector = web3.eth.abi.encodeFunctionSignature('transferERC3525Exact(bytes,address[7],uint8,uint256[6],bytes)')
		...
		const sig = await exchange.sign(one, account_a)
		const firstERC20Call = erc20c.methods.transferFrom(account_a, account_b, price*sellValue).encodeABI()
		const firstData = atomicizerc.methods.atomicize(
			[erc20.address],
			[0],
		...
		const seconderc3525call = erc3525c.methods.safeTransferFrom(account_b, account_a, tokenId, sellValue, "0x").encodeABI() + ZERO_BYTES32.substr(2)
		const secondData = atomicizerc.methods.atomicize(
			[erc3525.address],
			[0],
			[(seconderc3525call.length - 2) / 2],
		...
		assert.isTrue(to_balance.toNumber() > 0,'Incorrect balance')
		assert.equal(to_balance.toNumber(),  sellValue*fillCount, 'Incorrect balance')
		assert.equal(await erc20.balanceOf(account_b), price*fillCount*sellValue, 'Incorrect balance')
		}
		it('matches erc3525 <> erc20 signed orders, matched right, real static call',async () => 
		{
		return erc3525_erc20_match_right_static_call(100,5)
		})
		*/

		it('matches erc20 <> erc3525 signed orders, matched right, real static call',async () => 
		{
		return erc20_erc3525_match_right_static_call(100,5)
		})
		/*
