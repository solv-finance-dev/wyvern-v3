...
contract('WyvernExchange', (accounts) =>
	...
	it('matches erc3525 voucher-voucher swap order',async () =>
		...
		await Promise.all([erc3525.mint(vouchers[0].slot,vouchers[0].tokenId,vouchers[0].balance, {from: account_a}),erc3525.mint(vouchers[1].slot, vouchers[0].tokenId, vouchers[1].balance, {from: account_b})])
		...
		const firstData = erc3525c.methods.safeTransferFrom(account_a, account_b, vouchers[0].tokenId, vouchers[0].value, "0x").encodeABI() + ZERO_BYTES32.substr(2)
		const secondData = erc3525c.methods.safeTransferFrom(account_b, account_a, vouchers[1].tokenId, vouchers[1].value, "0x").encodeABI() + ZERO_BYTES32.substr(2)
